...
*Setup instructions for a gitian build of LFN Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LFN Core
...
- [Building LFN Core](#building-leafn0de)
...
Clone the git repositories for leafn0de and gitian and then checkout the leafn0de version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the leafn0de repository
...
To build LFN Core (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the leafn0de repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/leafn0de/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/leafn0de/leafn0de
...
For example:
```bash
URL=https://github.com/konez2k/leafn0de.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit leafn0de=${COMMIT} --url leafn0de=${URL} ../leafn0de/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit leafn0de=${COMMIT} --url leafn0de=${URL} ../leafn0de/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit leafn0de=${COMMIT} --url leafn0de=${URL} ../leafn0de/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/leafn0de-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/leafn0de-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/leafn0de-build.assert
```
...
[leafn0de/gitian.sigs](https://github.com/leafn0de/gitian.sigs/) repository, or if that's not possible to create a pull
request.
