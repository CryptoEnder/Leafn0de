...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("Leafn0deMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("Leafn0deMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("Leafn0deMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("leafn0de-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Leafn0deMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
