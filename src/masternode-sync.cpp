...
{
    LogPrintf("CMasternodeSync::GetNextAsset() %d\n", RequestedMasternodeAssets);
    ...
    RequestedMasternodeAttempt = 0;
    nAssetSyncStarted = GetTime();
    LogPrintf("CMasternodeSync::GetNextAsset() end %d\n", RequestedMasternodeAssets);
}
...
void CMasternodeSync::Process()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        ...
        if (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) {
            LogPrint("masternode", "CMasternodeSync::Process() - RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS %d\n", RequestedMasternodeAssets);
            if (pnode->HasFulfilledRequest("getspork")) continue;
            pnode->FulfilledRequest("getspork");

            LogPrint("masternode", "CMasternodeSync::Process() - RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS %d attempts %d\n", RequestedMasternodeAssets, RequestedMasternodeAttempt);
            pnode->PushMessage("getsporks"); //get current network sporks
