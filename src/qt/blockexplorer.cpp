...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::LFN, nValue);
...
std::string AddressToString(const CBitcoinAddress& Address)
{
    ...
    */
    ...
    std::string Content;
    Content += "<h1 style='color:#black;'>" + _("Transactions to/from") + "&nbsp;<span>" + Address.ToString() + "</span></h1>";
    Content += TxContent;
    return Content;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (leafn0de.conf).");
            QMessageBox::warning(this, "LFN Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
